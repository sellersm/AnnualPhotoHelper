...
#End Region
    ...
    Private Sub Form1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        'MSellers 6-20-2018 added the following to cause the Http connection to use TLS 1.2 since Blackbaud has deprecated the older 1.0:
    ...
    Private Sub Button_GetSourceFolder_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button_GetSourceFolder.Click
        ...
        If FolderBrowser_GetSourceFolder.ShowDialog = DialogResult.OK Then
    ...
    Private Function CreatePhotoCollectionFromReply(ByVal Reply As ServiceProxy.DataListLoadReply) As List(Of ChildPhotoData)
        ...
        For Each row As Blackbaud.AppFx.WebAPI.ServiceProxy.DataListResultRow In Reply.Rows
            returnList.Add(New ChildPhotoData(row.Values(lookupIdColumn).ToString(), row.Values(projectIdColumn).ToString(),
                      row.Values(childNameColumn).ToString().Replace(" ", ""), "", row.Values(crmNameColumn).ToString(),
        ...
        For Each fl As FileInfo In allFiles
            ...
            If isValidToParse = True AndAlso CountCharacter(fl.ToString().Substring(0, 16), "-") = 1 Then
                ...
                Else
                    ...
                    'MSellers 5-16-2018 added check in case the filename has no spaces whatsoever!
                    If fileNameItems.Count() <= 1 Then
                        Throw New Exception(String.Format("There is at least one invalid photo file that has no spaces in the filename: {0}!", fileNameItems(0)))
                    End If
